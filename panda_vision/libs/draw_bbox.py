import fitz
from panda_vision.config.constants import CROSS_PAGE
from panda_vision.config.ocr_content_type import BlockType, CategoryId, ContentType
...
def draw_bbox_with_number(i, bbox_list, page, rgb_config, fill_config, draw_bbox=True):
    ...
    for j, bbox in enumerate(page_data):
        x0, y0, x1, y1 = bbox
        rect_coords = fitz.Rect(x0, y0, x1, y1)  # Définit le rectangle
        if draw_bbox:
            if fill_config:
                page.draw_rect(
                    rect_coords,
        ...
        )  # Insère l'index dans le coin supérieur gauche du rectangle
...
def draw_layout_bbox(pdf_info, pdf_bytes, out_path, filename):
    ...
    for i, page in enumerate(pdf_docs):
        ...
        # draw_bbox_without_number(i, tables_list, page, [153, 153, 0], True)  # couleur !
...
def draw_span_bbox(pdf_info, pdf_bytes, out_path, filename):
    ...
    for page in pdf_info:
        ...
        # Déplace les spans qui traversent les pages dans la liste de la page suivante
        ...
        # Construit les autres useful_list
        # Pour les spans, on peut utiliser directement le résultat avant la fusion des paragraphes
    ...
    pdf_docs = fitz.open('pdf', pdf_bytes)
    for i, page in enumerate(pdf_docs):
        # Récupère les données de la page courante
